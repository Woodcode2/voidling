...
export function worldRect(f: Foot, wx: number, wy: number, ry: number, strict = false):
{ cx: number; cz: number; hx: number; hz: number; c: number; s: number; strict?: boolean } {
  const c = Math.cos(ry), s = Math.sin(ry);
  ...
  return { cx: wx + (f.cx * c + f.cz * s) * 20, cz: wy + (-f.cx * s + f.cz * c) * 20,
    hx: f.hx * 20, hz: f.hz * 20, c, s, strict: strict || undefined };
}
