...
const STEPS = [0, 0.22, 0.35, 0.5, 0.7, 1.0];
...
await p.evaluate(() => { window.__setVoidR(6); window.__setMood('cruise'); });
...
await p.waitForFunction(() => (window.__faceState?.().maw ?? 1) < 0.02, null,
  { timeout: 300000, polling: 150 });
...
for (const v of STEPS) {
  await p.evaluate((k) => window.__pinGape(k), v);
  await p.waitForTimeout(900);
  const name = `gape-${String(v).replace('.', 'p')}.png`;
  await p.screenshot({ path: `${OUT}/${name}`, clip: box });
  const st = await p.evaluate(() => window.__faceState());
  console.log(`  ${name.padEnd(18)} smile ${st.smile ? 'shown ' : 'hidden'}  maw(mood) ${st.maw.toFixed(2)}`);
}
