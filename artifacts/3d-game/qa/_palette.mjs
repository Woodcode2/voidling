...
for (const wid of WORLDS) {
  ...
  const out = await p.evaluate(({ DH, PH, DT, PT }) => {
    ...
    window.__scene.traverse(o => {
      ...
      const rec = byHex.get(hex) || { n: 0, area: 0, h: 0 };
    ...
    const sum = (list) => list.reduce((a, h) => {
      const r = byHex.get(h); return { n: a.n + (r?.n || 0), area: a.area + (r?.area || 0) }; }, { n: 0, area: 0 });
    ...
    return { totalArea: Math.round(totalArea), distinct: byHex.size,
      darkHouse: sum(DH), paleHouse: sum(PH), darkTower: sum(DT), paleTower: sum(PT),
      bins: bins.map(v => +(v / totalArea * 100).toFixed(1)), top };
