// _thumbwall.mjs — a CHILD'S THUMB, not a perfect driver.
//
...
await p.evaluate(({ MODE, SEED }) => {
  ...
  const tick = () => {
    ...
    if (prev) {
      ...
      // COAST CONTACT: is any point on the void's rim over illegal ground?
      // prototype3d.ts:4120+ cancels only the OUTWARD half of the velocity, so a
      // void touching the shore SLIDES instead of going where it is pointed.
      let touching = false;
      for (let a2 = 0; a2 < 8; a2++) { const th = a2 * Math.PI / 4;
        if (!window.__biomeAt(vs.x + Math.cos(th) * vs.r, vs.z + Math.sin(th) * vs.r)) { touching = true; break; } }
      A.tTouch = (A.tTouch || 0) + (touching ? dt : 0);
      if (gripped) {
        ...
        if (spd > 0.5 && sm > 1) {
          ...
          if (eS != null) { A.errStick.push(eS); A.binsStick[Math.min(18, Math.floor(eS / 10))] += dt;
            ...
            if (touching) { (A.errTouch ||= []).push(eS); A.mTouch = (A.mTouch || 0) + dt;
              if (eS > 45) A.tw45T = (A.tw45T || 0) + dt; if (eS > 90) A.tw90T = (A.tw90T || 0) + dt;
              A.spdTouch = (A.spdTouch || 0) + spd * dt; }
            else { (A.errOpen ||= []).push(eS); A.mOpen = (A.mOpen || 0) + dt;
              if (eS > 45) A.tw45O = (A.tw45O || 0) + dt; if (eS > 90) A.tw90O = (A.tw90O || 0) + dt;
              A.spdOpen = (A.spdOpen || 0) + spd * dt; } }
  ...
  window.__grab = () => {
    ...
    return { t: +ms.t.toFixed(1), score: ms.score, r: +ms.r.toFixed(2),
      ...
      pctTimeTouchingCoast: +(100 * (A.tTouch || 0) / (A.gt || 1)).toFixed(1),
      atCoast: { err: q(A.errTouch || []), movingT: +(A.mTouch || 0).toFixed(1),
        pctWrong45: +(100 * (A.tw45T || 0) / (A.mTouch || 1)).toFixed(1),
        pctWrong90: +(100 * (A.tw90T || 0) / (A.mTouch || 1)).toFixed(1),
        meanSpd: +((A.spdTouch || 0) / (A.mTouch || 1)).toFixed(1) },
      inOpen: { err: q(A.errOpen || []), movingT: +(A.mOpen || 0).toFixed(1),
        pctWrong45: +(100 * (A.tw45O || 0) / (A.mOpen || 1)).toFixed(1),
        pctWrong90: +(100 * (A.tw90O || 0) / (A.mOpen || 1)).toFixed(1),
        meanSpd: +((A.spdOpen || 0) / (A.mOpen || 1)).toFixed(1) },
