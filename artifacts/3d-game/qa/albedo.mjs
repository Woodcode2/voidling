...
// saturated ones (dominant channel > 0.25 linear) whose weakest OTHER channel
// sits under the bar. Source census only: the render check (luminance range
...
for (const f of files) {
  ...
  lines.forEach((ln, i) => {
    ...
    for (const m of ln.matchAll(/0x([0-9a-fA-F]{6})\b|'#([0-9a-fA-F]{6})'|"#([0-9a-fA-F]{6})"/g)) {
      ...
      const others = L.filter((v) => v !== mx);
      const ratio = Math.min(...others) / mx;            // the weakest secondary channel
...
console.log('  hex      weakest/dominant  sites');
