...
const SUITE = [
  ...
  // every WebGL program linked, or first drawn, after the first playable frame
  // of a real match on the phone's path (poster menu, a touch to start). Live and quality until
  ...
  { id: 'shaderwarm', tier: 'feel', profiles: ['live', 'quality'], timeout: 4800,
    ...
    why: 'no shader is compiled on first sight during play — every program a match needs (the family, every form, the effects, the end card) is linked and first drawn before the first playable frame, so nothing new on screen can freeze the game' },
