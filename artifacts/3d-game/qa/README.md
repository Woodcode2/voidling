...
| `joyedge.mjs` | **The bezel.** Lands a thumb 12/20/30/45px from the right edge, pushes toward it and away from it, and compares how far the void gets against the MATCH clock. Those two numbers should match. On the pre-fix build the 12px case moved 0.01u against 21.77u — the void did not move at all — and 45px, an ordinary one-handed grip, got 61%. |
| `joyrelease.mjs` | Every way a drive ends. Drives with a real pointer, then takes the thumb away four ways — lift, app backgrounded, tab hidden, pause sheet held through RESUME — and asks whether the void stops. Skips a coast window before sampling: velocity is smoothed downstream of the stick, so measuring from the instant of release scores a working release and a broken one identically. |
...
| `newsarc.mjs` | **Does the town tell a story?** Plays a real match and asserts the four phases appear in order and never reverse, that morning owns the first two cards and never mentions the void, that no card carries a `💬` chip or a family name, that nothing repeats, that every card was **measured on screen**, that eating a real landmark gets it named in the paper, and that PLAY AGAIN starts the story over. `ARC_WORLD=pirate node qa/newsarc.mjs`. See `docs/NEWSROOM-BRIEF.md`. |
| `newsstyle.mjs` | The house style as a checked property, not prose: the punctuation ladder per beat, the token vocabulary, the 78-char cap at worst-case token fill, and the corpus shape (≤45% two-sentence lines, ≥35% one-sentence, some real questions). Covers all four newsrooms plus the reactive pools. |
