...
// heroswatch prints e.g. `    3   117, 71,173    #7547ad   0.587   0.677` for radius 3
const hero = (f) => { try { const m = fs.readFileSync(f, 'utf8').match(/^\s*3(?:\.0+)?\s+(\d+),\s*(\d+),\s*(\d+)/m); return m ? [+m[1], +m[2], +m[3]] : null; } catch { return null; } };
