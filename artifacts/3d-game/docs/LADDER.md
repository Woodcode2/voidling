...
|---|---|---|---|---|---|---|
| 1 | Lantern thumb legibility | GRAPHICS | ~~S~~ M | M | High — measured fail | **ATTEMPTED & RETRACTED**: a lantern-only +55% hemisphere floor moved the 120px crop by +0.4 mean luminance / −1.1pt dark share (qa/out/lantern_thumb_ab.png) — nothing. The murk is albedo-bound (night ground authored near-black); the honest fix is a ground-bake albedo pass in island.ts. Re-scoped M, queued next session. |
