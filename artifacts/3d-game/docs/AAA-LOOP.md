...
| S-4 | The hook: a 15-second store clip (the burp after eating the town) and App Store screenshots | **ready for the Mac day.** Screenshots: `pnpm shoot:store` now takes nine (Skylark added) and runs on a Mac; rehearsed end to end here (SHOOT_DRYRUN) and three shots fixed from what the rehearsal showed — 02 photographed a town (PLAY starts Maple since G1; the picker's door is #worldSwitch), 08 a TRY AGAIN card (now a win), 09 mostly grass (now the launch rows). The committed store/*.png are stale (9 Sep) and are re-shot on the Mac after the art is vendored. The preview is his iPhone screen recording of the TestFlight build, cut to 15-30 s in iMovie (APPSTORE.md); store/preview.mp4 is the retired 2D game and must not be uploaded. |
| S-5 | His calls on the burp (?burp=1), the kill beat (?killbeat=1), the after-bite squint (flat lines vs ^ ^), the too-big bonk rate — and G2 (the streak skins show in store shot 07) and G12 | **needed BEFORE the iOS build**: the app has no address bar, so whatever ships ON is what the preview and screenshots can show |
...
| S-8 | **The owner's recording, 2026-09-25** (Skylark in Safari, iPhone 17 Pro Max): a 950 ms freeze at 14.59 s (56.7 fps effective over 36 s; hitches of 133 and 83 ms) — the game never pre-compiles a shader; the growth bar steps in blocks; the NOMS pill beside the void takes room (move it to a top corner); remove the events (banners + x2/x3 windows), news only in the morning and at milestones; the menu should come back on the world she chose; menu music does not start until after a match | building (workflow owner-0925-fixes: lag, hud, events, menu) |
| S-9 | **Campaign vs free play:** campaign (the 30 levels) earns currency; picking a world to "just play" earns none. **New Game Plus** after level 30: back to the start, less time, higher goals, new things to earn | owner's idea 2026-09-25 — proposal sent, awaiting his OK |
| S-10 | **Skylark becomes a cloud kingdom** (his idea, after One Piece's sky island): clouds as the ground, old sky castles, sky folk, a GIANT BELL at the centre as the hero landmark — our own design and names, not theirs | proposal sent, awaiting his OK; he sources its music |
| S-11 | **The Voidling family story**: *"it's almost like we're just throwing everything at the wall … I'd love for this to somehow be a story … about the Voidlings and their family … what's their purpose? … creating some sense of family or some sense of adventure. That's the element we're missing."* | proposal sent, awaiting his OK |
