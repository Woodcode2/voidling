...
| B3, B4 / Job 4 | Walking people's eyes buried by hair shells and caps | MOTION | **Geometry DONE, frame read OWED — B3/B4 stay open until a person reads the I-3 line-up** — qa/faceray.mjs (new, node: runs life.ts's own head code through the TS compiler and raycasts each eye from 46/55/65°): (E) before **FAIL**, 120 of 126 Hair x Hat under 50% of an eye (shared crown 0/0/0 on 10 of 14 hair draws, buzz 43/42/41, curly 55/53/52, every hat 0/0/0); after **PASS**, 162 built, lowest reading outside the conventions 69% (beanie at 65°). Hair shells −0.55 (the governor's number), cap/postal crown and the skeptic's visor as specified; beanie, bandana, visor, bucket and tricorn tipped rigidly, hood and flower tipped in part, helmet and shako re-seated (each value, its rejected neighbour and what was re-authored is in the life.ts comment). Sun, straw, captain exempt by the governor's convention and nothing else. **Review round (fix commit):** the snorkel exemption the first commit took on its own authority is withdrawn — the mask now sits up on the forehead (snorkel 0/0/0 → 100/100/98, mouth 96/62/22 → 100); the helmet's white face bar, left floating across the chin when the helmet tipped back, is deleted (mouth 88/90/96 → 100, −12 tris); a mouth ratchet (M) now gates what no probe saw (floor 88%, this build's lowest: bob under the tricorn at 65°); (H) now covers every colour a role authors as well as the dress-code pick — on the first commit 1771 of 31478 outcomes were within dE 15, 297 on pairs a role really builds (the protester's cream cap on a cream shirt, dE 0; cream sun hats on the palest skin, 7.1), now 0 — by re-shading a failing colour to its nearest shade that clears, so uniforms and campaign caps keep their hue; faceline's subject tip ran the wrong way (checked in node: 46/55/65 → 37.8/10.0/−20.1) and is fixed; every Job 4 probe prints FAIL on an abort. Curly under a hat is drawn short: −840 tris under the crowned hats (5 × 168; the spec's 960 was 2·W·H), and a visible loss of curls under the open ones (visor, flower, snorkel), as the spec says "any hat". qa/_headcover.mjs hardened (eye parsed, ninth argument parsed, exits 1): before **FAIL** (crown and hood swallow the eye), after **PASS**, scalp still 0.0% bare. faceray and headcover in push; faceline registered in live and quality only (never run). **Owed:** `node qa/faceline.mjs 4177 maple` and a person reading its frames; the shippedlook reshoot of all six worlds |
