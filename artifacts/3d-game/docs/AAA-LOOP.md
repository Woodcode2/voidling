...
| B7 / Job 9 | Lantern Night's lanterns light up; umbrella and moss rock | LIGHT + STATIC | **node half landed on branch, browser half OWED** — qa/emitters.mjs: 20 of 20 lamp colours under 1.2x their world's cut at its brightest hour (0.29x-1.10x) before, all at 1.29x after (the floor rides the per-world, per-hour cut; gain cap 5.0, Pirate's pink needs 4.44). qa/propfit.mjs, 200 builds, before (26a7de8) -> after: ribs outside the paper 49.0% -> 0; pole line 16.1 radii off the apex -> 0; foot 0.059 off the ground -> under it; pole top through the paper 6859/7200 -> 0 (U4); umbrella reach from its placement origin 1.043x its rim radius -> 1.000x (U5 — the first lean about the foot had it at 1.58x, 1.85 units, so the umbrella is slid back until the canopy's rim centre is on the origin); moss outside the rock 0.0% -> 72.2% lowest (M1); moss above the rock's highest point, **the studio's literal wording, barred per build** 0.0% -> 31.1% lowest (28.1% lowest of 3,000; M3); moss rim over the rock's surface 0.110 of its height worst -> 0.000 (M4 — the squash alone left a lid at 0.298, so the cap is laid on the rock). **Owed:** qa/lampglow.mjs has never run (live + quality only, out of push until read) — lantern and powder on 26a7de8 and this tree, with the hue table (red paper must read red after ACES); halocensus and postpipe --gate on Lantern, Powder, Pirate, Skylark; A/B frames for a person: the four lanterns on the wire, Pirate's neon, Skylark's runway lights, a crop of a leaning umbrella (and whether a canopy meets a neighbouring stall), a crop of a moss rock, and one rung-2 frame per lamp world; qa/placement.mjs lantern (the umbrella's ground slice moved with the slide). |
