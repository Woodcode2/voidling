export const meta = {
  name: 'firstframe-studio-continue',
  description: 'Stream D continuation after the session limit — the seven recorded reviews refuted from disk, the hero reviewed and refuted, art direction, the order of work: five establishing-shot cinematographers, the splash UI, choreography and the hero, each refuted by a skeptic; art direction; the governor\'s order of work',
  phases: [
...
phase('Review')
// Seven reviews were recorded before the session limit killed the run (each
// author pushed its own file: docs/crews/round-5/firstframe/review-<key>.md).
// They are read back by a cheap agent rather than re-reviewed; the hero is the
// one surface never reviewed.
const RECORDED = SURFACES.filter((s) => s.key !== 'hero')
const readReport = (s) => agent(`Return, verbatim and complete, the contents of the file ${REPORTS}/review-${s.key}.md (use the Read tool; if the file is longer than one Read, keep reading until the end). Output ONLY the file's contents, nothing else — no preamble, no commentary.`,
  { label: `read:${s.key}`, phase: 'Review', effort: 'low' })
  .then((text) => ({ key: s.key, name: s.name, images: s.images, text: text || '' }))
const heroSurface = SURFACES.find((s) => s.key === 'hero')
const reviewHero = () => agent(`${CHARTER}

═══════════════════════════════════════════════════════════════════
YOU ARE ${heroSurface.name}
═══════════════════════════════════════════════════════════════════

${heroSurface.brief}

IMAGES YOU MUST READ BEFORE WRITING ANYTHING (absolute paths):
${heroSurface.images.map((i) => `  ${i}`).join('\n')}

NUMBERS YOU MUST READ:
${heroSurface.data.map((d) => `  ${d}`).join('\n')}

YOUR REPORT PATH: ${REPORTS}/review-hero.md  (repo-relative: artifacts/3d-game/docs/crews/round-5/firstframe/review-hero.md)
Record it with qa/_record.sh before you return.`,
  { label: 'review:hero', phase: 'Review', effort: 'high' })
  .then((text) => ({ key: 'hero', name: heroSurface.name, images: heroSurface.images, text: text || '' }))
...
log(`Continuation: ${RECORDED.length} recorded reviews to refute, the hero to review and refute.`)
const reviewed = await parallel([
  ...RECORDED.map((s) => () => readReport(s).then(refute)),
  () => reviewHero().then(refute),
])
