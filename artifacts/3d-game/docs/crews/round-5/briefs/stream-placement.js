...
failures that are not findings. Acquire, run, release in ONE shell command:
  while ! mkdir /tmp/gpu.lock 2>/dev/null; do
    ...
    sleep 5; done; node qa/yourprobe.mjs ARGS; rmdir /tmp/gpu.lock
...
TOKEN CAUTION (the owner is watching spend): do not re-read files you do not
need; read GOVERNOR.md for its HANDS OFF list and ledger headings, not the
whole file; docs/STUDIO.md only for the CREWS pipeline section; the brief only
for your stream. Photograph what matters, not everything. The lock poll is 5 s.
The container restarts without warning: your proposal file in MAIN is swept into
git every 15 minutes by the governor — write to it early and often.
...
const ONLY = LANES.filter((l) => l.key === 'placement')
log(`running ${ONLY.length} lane(s): ${ONLY.map((l) => l.key).join(", ")}`)
...
const out = {}
ONLY.forEach((l, i) => { out[l.key] = results[i] })
const dead = ONLY.filter((l, i) => !results[i]).map((l) => l.key)
