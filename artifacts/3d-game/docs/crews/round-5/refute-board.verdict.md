...
- End-screen stage of refute_board.mjs TIMED OUT (900 s) waiting for `#end.show` after `__rushClock(6)` at t=60.5 — the loop was alive (t advanced 0.5 match-s in 2.5 s wall just before), so this is either a whistle gate my recipe missed or a stall; re-running the end screen alone with qa/_mvhud.mjs's exact recipe (`__setVoidR(9); __rushClock(8)`) and 20 s diagnostics (scratchpad/refute_board_end.mjs), queued behind qa/solotog.mjs + qa/uisystem.mjs which hold the lock since 11:03:46.
- `node qa/solotog.mjs 4177` (11:03:46-11:0x): ALL PASS, exit 0 — 8 checks: default rivals-on; normal run clock=160s; normal run 2 joined of 4; toggle on; solo clock=100s; solo 0 joined of 5; voidSolo=1 stored; survives reload. No null deref, no throw.
- `node qa/uisystem.mjs 4177`: PASS — "every weight is a real face, every size is readable", exit 0 (menu, picker, shop, settings).
