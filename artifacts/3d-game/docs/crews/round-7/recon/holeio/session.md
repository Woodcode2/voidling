...
- 10:20 **Resume point (owner: "approaching limit").** State: §11 (M1) in the report; `holeio.polish-plan.md` draft 1 pushed with our measured Maple + menu baseline (`recon/self/`); M3 landed and is saved raw in `vid-M3-findings.md` (not yet written into §11.9). Still running when cut: M2 (swallow/floaters/size-up), M4 (audio), skeptic — workflow `wf_1d8d7f37-7dc`, task `wwm744ii0`; if the container survived, read its `journal.jsonl` for `result` rows; if not, re-run the workflow script `workflows/scripts/holeio-video-measure-wf_1d8d7f37-7dc.js` after re-extracting frames (command above). Five-world baseline shots: `qa/_worldshots.mjs pirate` exited 1 (unlock state or script arg — not investigated); Maple only is measured. Next: write §11.8–11.11 from M2/M3/M4 + skeptic, fill the † placeholders in plan stream E, re-issue as draft 2, then the owner decides plan §4 before any crew is briefed.
- 16:10 M2 (swallow/floaters/size-up) and M4 (audio) landed; written into the report as §11.8 and §11.10, raw returns saved as `vid-M2-findings.md` / `vid-M4-findings.md`. The skeptic failed twice on account limits (session limit, then credits) — recorded as §11.11: **no number in §11 has been adversarially re-measured.** All six worlds' baseline frames are in `recon/self/` with a comparison table in the plan. Plan is draft 2, complete, no placeholders. Owner's six decisions are in plan §4. Operating model agreed: the governor writes briefs and gives skeptic verdicts, Opus at max effort builds and probes.
- 20:40 **Stream A opened.** Brief written (`streamA.opening-brief.md`): diagnosis, six measured weaknesses in their opening, the four things a hole cannot do (arrival / guaranteed first bite / descent lands on the hero / sound), 22 bars, order of work. `qa/opening.mjs` written BEFORE the build and run on the untouched tree: 8 of 10 bars fail; A1 = 0.550 s of match clock burned before the first touch. **Instrument correction recorded in §6.1**: the first run's wall-clock durations were invalid under swiftshader (10.9 s reported for a 2.2 s intro) because the loop clamps dt; the probe now measures on the match clock. **The verify+locate workflow (wf_8c07db18-e01) failed ALL EIGHT agents on the session limit** — five skeptics and three code scouts, zero results. The skeptic pass on §11 is still owed, and the brief's code-site table (§ between bars and risks) is still missing: re-run with `Workflow({scriptPath:'/root/.claude/projects/-home-user-voidling/2a240c97-4d5b-5823-ac36-95bed2f17b29/workflows/scripts/holeio-verify-and-locate-wf_8c07db18-e01.js', resumeFromRunId:'wf_8c07db18-e01'})` when limits reset. Container has 4 CPUs so the workflow cap is 2 concurrent agents.
- 21:10 Probe's before-run is honest after **three** instrument corrections (wall-clock vs game clock; `?w=` booting the world so the intro ran during the menu clicks; the camera jumping to its intro height rather than starting there). All three recorded in the brief §6.1. Real before-numbers on Maple: A1 0.536 s of clock before touch, A4 283 ms, A5 ≥1819 ms, A6 ease-out-quad, A6b 0.679, **A7 ×5.31 PASSES**, A8 4 dead frames, A9 no floater, A21 669 ms early-vs-late difference. Log at `recon/self/opening-before.log`.
